                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            f"💡 REGRA DE CONTAGEM GERAL E QUANTIDADE DETERMINÍSTICA (MUITO IMPORTANTE):\n"
                            f"Muitas vezes, a imagem contém múltiplos módulos idênticos enfileirados/adjacentes um ao lado do outro (ex: 3 estantes de 'PF 807mm' ou 2 estantes de 'MED 500mm'). O modelo de visão costuma ter preguiça e ignorar um ou outro módulo, contando uma quantidade menor do que a real. Para evitar isso, siga esta regra sagrada:\n"
                            ...
                            f"- Se a etiqueta de texto estiver impressa **TRÊS** vezes na imagem (por exemplo, três caixas ou colunas contendo o texto 'PF 807mm'), a quantidade ('qtd') desse item no JSON DEVE ser **3**! Nunca retorne '2' se vir 3 textos!\n"
                            ...
                            f"- Nunca agrupe módulos que possuem textos/etiquetas distintas escritas reduzindo a quantidade. Cada tag de texto no desenho representa 1 unidade física daquele móvel. Se houver 3 textos 'PF 807mm', a quantidade é exatamente 3!\n\n"
                            ...
                            f"   - **⚠️ DISTINÇÃO ABSOLUTA E EVITAÇÃO DE CONFUSÃO COM ESMALTES:** Um **CESTAO 400** (Cestão 400x400) é desenhado no CAD como um quadrado isolado dividido em 4 quadrantes iguais (um grid de 2x2). Dentro de cada quadrante, há desenhos de produtos circulares (que representam frascos/potes de produtos) e retangulares (que representam caixas), e a escrita pixelada (como 'RESTAG 400' ou 'CESTÃO 400'). Devido à presença de pequenos círculos organizados, o Vision AI costuma se confundir e ler isso como 'ESMALTES' (que possui pequenos círculos ou quadrados de vidrinhos). **ESSA CONFUSÃO É UM ERRO GRAVE!** Se você vir um quadrado isolado no desenho dividido em 4 quadrantes (grid 2x2) com frascos/produtos desenhados, **ELE É OBRIGATORIAMENTE UM CESTAO 400 com acabamento 'normal'!** Nunca classifique-o como 'ESMALTES', 'OUTROS ITENS' ou qualquer outro móvel, mesmo que o texto pareça ilegível ou pareça sugerir outra palavra. O expositor de 'ESMALTES' é sempre uma estante de parede linear, retangular e comprida, nunca um quadrado promocional isolado 2x2.\n"
